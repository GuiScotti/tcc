    ...
    <div class="offcanvas-body">
      <div class="inputAcc d-flex flex-column w-100 rounded p-1">
        ...
        <div class="d-flex flex-column w-100 justify-content-between mt-3">
          ...
          <div class="w-100" >
            <div class="gap-2 d-flex flex-row" style="overflow-x: auto;">
...
export default {
  ...
  methods: {
    ...
    ...mapActions("contacts", ["createContact", "setContacts"]),
    ...mapActions("stages", ["setStages"]),
    async createNewContact() {
      const response = await this.createContact({
        funnel_id: Number(this.funnel.id),
        contact: {
          position: this.getContactsByStage(this.chosen_stage_id).length + 1,
          name: this.name,
          funnel_id: Number(this.funnel.id),
          stage_id: Number(this.chosen_stage_id),
          email: this.email,
