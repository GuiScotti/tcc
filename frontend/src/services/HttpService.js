...
export const resetPassword = (email) => {
  return HttpService.post(`/password.reset`, { email });
};
...
export const getMetrics = async (token) => {
  return await HttpService.get("/funnels/metrics", {
    headers: {
      Authorization: `Bearer ${token}`,
    },
  });
...
export const updateContact = async (funnel_id, contact_id, contact, token) => {
...
export const swap = async (funnel_id, contact_id, stage_id, changes, token) => {
  return await HttpService.put(
    `${funnel_id}/contacts/${stage_id}/swap/${contact_id}`,
    changes,
    {
      headers: {
        Authorization: `Bearer ${token}`,
